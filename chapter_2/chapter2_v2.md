...
### 2.2 管翅式
&emsp;&emsp;翅片式器的基本结构是管道插入翅片或者翅片环绕盘管,常用的结构类型有：
+ 管翅式（管穿翅）
+ 旋翅式 （翅绕管）
+ 微通道式 （管夹翅）

&emsp;&emsp;这三种类型的换热器基本结构都是管道和翅片过盈配合，将翅片和管道间的接触热阻降至最低，使翅片成为高效的强化传热表面，从而起到减小空气侧热阻，增强换热性能的作用。
&emsp;&emsp;这三种蒸发器在冰箱中的应用场景：管翅式常用于冰箱系统的箱内蒸发器；而旋翅式和微通道式常用于大冰箱的冷凝器，装在压缩机仓内。针对不同的应用场景，冰箱的换热器的设计原则和性能要求是有区别的：

#### 2.2.1 穿入翅片式
&emsp;&emsp;穿入式翅片通常用作冰箱的蒸发器，常见的有两种结构：
+ 斜插式
+ 拉胀式 
这两种结构的翅片换热器原理相似，都是靠过盈配合让管道和翅片紧密连接，接触热阻可以忽略不计，依靠翅片增加空气侧传热面积，起到减小空气侧传热热阻的作用。
&emsp;&emsp;两种翅片式的蒸发器的性能是非常接近的，但同等的蒸发器体积下，两种蒸发器的换热性能非常接近。
